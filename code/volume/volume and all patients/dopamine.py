...
for row, mean, df_rec_plot, common, label, color in datasets:
    for col, receptor in enumerate(receptor_cols, start=1):
        ...
        fig.add_trace(go.Scatter(
            ...
            text=region_names,
            hovertemplate='<b>%{text}</b><br>Volume: %{x:.4f}<br>Receptor: %{y:.4f}<extra></extra>',
        ...
        fig.update_xaxes(title_text=' Volume', row=row, col=col)
...
fig.update_layout(
    title='Spearman ρ: Volume vs Receptor Density',
    height=800, width=1200
)

fig.write_html('volume_Dopamine_all_patients.html')
fig.show()