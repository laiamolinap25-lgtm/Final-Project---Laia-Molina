...
# Mean volume per region
mean_ctx = pd.concat([control_decline[cols_ctx], mci_decline[cols_ctx]]).mean()
mean_ctx.index = mean_ctx.index.str.replace('_grayvol', '')

mean_sub = pd.concat([control_decline[cols_sub], mci_decline[cols_sub]]).mean()
...
fig, axes = plt.subplots(2, 2, figsize=(15, 10), sharex='row', sharey='row')
...
plt.suptitle('Spearman ρ: gray volume vs receptor density', fontsize=12)
plt.tight_layout()
plt.show()