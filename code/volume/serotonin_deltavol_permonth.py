...
for i, receptor in enumerate(receptor_cols):
    row = i // 3 + 1
    col = i % 3 + 1
    ...
    fig.layout.annotations[i].text = f'{receptor} | ρ={rho:.3f}'
...
fig.update_xaxes(title_text='Δ gray volume (mm³/month)', row=2)
fig.update_yaxes(title_text='z-score', col=1)

fig.show()
fig.write_html(r"C:\Users\laiam\OneDrive\Escritorio\Practicas Canada\neurotransmitters\data\volume_correlation\deltavol_correlation_serotonin_permonth.html")