...
t1t2 = pd.concat(dfs, ignore_index=True)

t1t2["tp_months"] = t1t2["timepoint"].map(timepoint_to_months)

# Check unknown timepoints
unknown = t1t2.loc[t1t2["tp_months"].isna(), "timepoint"].unique()
print("Unknown timepoints:", unknown)
...
patient_summary.to_csv(
    os.path.join(output_dir, "t1t2_patient_summary_by_group.csv"),
    index=False
)
...
summary.to_csv(
    os.path.join(output_dir, "t1t2_patient_summary_group_level.csv"),
    index=False
)
...
timepoint_summary.to_csv(
    os.path.join(output_dir, "t1t2_timepoint_summary_by_group.csv"),
    index=False
)

# =========================
# PLOT 1: PATIENTS BASELINE VS DELTA
# =========================
...
plt.title("T1T2 patients valid for baseline and delta analyses")
plt.legend()
plt.tight_layout()
plt.savefig(
    os.path.join(output_dir, "t1t2_baseline_delta_valid_by_group.png"),
    dpi=300
)
plt.close()

# =========================
# PLOT 2: TIMEPOINT AVAILABILITY
# =========================
...
plt.title("T1T2 timepoint availability by diagnostic group")
plt.tight_layout()
plt.savefig(
    os.path.join(output_dir, "t1t2_timepoint_availability_by_group.png"),
    dpi=300
)
plt.close()

# =========================
# PLOT 3: VISITS PER PATIENT
# =========================
...
for group in visits_dist["group"].unique():
    ...
    plt.ylabel("Number of patients")
    plt.title(f"T1T2 visits per patient - {group}")
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, f"t1t2_visits_per_patient_{group}.png"),
        dpi=300
    )
    plt.close()
