...
for row, mean, df_rec_plot, common, label, color in datasets:
    for col, receptor in enumerate(receptor_cols, start=1):
        ...
        fig.add_trace(go.Scatter(
            ...
            text=region_names,
            hovertemplate='<b>%{text}</b><br>FDG: %{x:.4f}<br>Receptor: %{y:.4f}<extra></extra>',
...
fig.update_layout(
    title='Spearman ρ: FDG (baseline) vs Cholinergic Receptor Density — All Patients',
    height=800, width=1200
)

fig.write_html('baseline_fdg_ACh_all_patients.html')
fig.show()
