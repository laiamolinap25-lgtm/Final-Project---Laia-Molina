...
for row, mean, df_rec_plot, common, label, color in datasets:
    for col, receptor in enumerate(receptor_cols, start=1):
        ...
        fig.add_trace(go.Scatter(
            ...
            text=region_names,
            hovertemplate='<b>%{text}</b><br>Δ T1/T2: %{x:.4f}<br>Receptor: %{y:.4f}<extra></extra>',
        ...
        fig.update_xaxes(title_text='Δ T1/T2 (per year)', row=row, col=col)
...
fig.update_layout(
    title='Spearman ρ: Δ T1/T2 (per year) vs Dopaminergic Receptor Density — All Patients',
    height=800, width=1200
)

fig.write_html('delta_t1t2_DA_all_patients.html')
fig.show()