...
delta_sub = pd.concat([
    calc_baseline(df_control, cols_sub),
    calc_baseline(df_mci,     cols_sub),
    calc_baseline(df_ad,      cols_sub)
])
...
for row, mean, df_rec_plot, common, label, color in datasets:
    for col, receptor in enumerate(receptor_cols, start=1):
        ...
        fig.add_trace(go.Scatter(
            ...
            text=region_names,
            hovertemplate='<b>%{text}</b><br>T1/T1: %{x:.4f}<br>Receptor: %{y:.4f}<extra></extra>',
        ...
        fig.update_xaxes(title_text='T1/T2 (baseline)', row=row, col=col)
...
fig.update_layout(
    title='Spearman ρ: T1/T2 (baseline) vs Cholinergic Receptor Density — All Patients',
    height=800, width=1200
)

fig.write_html('baseline_t1t2_ACh_all_patients.html')
fig.show()
print(len(df_control.patient_id.unique()), len(df_mci.patient_id.unique()), len(df_ad.patient_id.unique()))