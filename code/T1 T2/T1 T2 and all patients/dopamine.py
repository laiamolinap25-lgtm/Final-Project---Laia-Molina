...
delta_ctx = pd.concat([
    calc_last_value(df_control, cols_ctx),
    calc_last_value(df_mci,     cols_ctx),
    calc_last_value(df_ad,      cols_ctx)   # ← añadido
])

delta_sub = pd.concat([
    calc_last_value(df_control, cols_sub),
    calc_last_value(df_mci,     cols_sub),
    calc_last_value(df_ad,      cols_sub)   # ← añadido
])
...
for row, mean, df_rec_plot, common, label, color in datasets:
    for col, receptor in enumerate(receptor_cols, start=1):
        ...
        fig.add_trace(go.Scatter(
            ...
            text=region_names,
            hovertemplate='<b>%{text}</b><br>T1/T2 baseline: %{x:.4f}<br>Receptor: %{y:.4f}<extra></extra>',
...
fig.update_layout(
    title='Spearman ρ: T1/T2 (baseline) vs Receptor Density',
    height=800, width=1200,
    template='plotly_white'
)

fig.write_html('baseline_t1t2_dopamine_all_patients_with_ad.html')
fig.show()