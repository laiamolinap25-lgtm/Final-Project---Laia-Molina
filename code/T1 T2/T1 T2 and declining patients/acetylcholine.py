...
delta_sub = pd.concat([
    calc_baseline(control_decline, cols_sub),
    calc_baseline(mci_decline,     cols_sub)
])
...
for row, mean, df_rec_plot, common, label, color in datasets:
    for col, receptor in enumerate(receptor_cols, start=1):
        ...
        fig.add_trace(go.Scatter(
            ...
            text=region_names,
            hovertemplate='<b>%{text}</b><br>T1/T2: %{x:.4f}<br>Receptor: %{y:.4f}<extra></extra>',
...
fig.update_layout(
    title='Spearman ρ: T1/T2 (baseline) vs  Receptor Density — Deteriorating Patients',
    height=800, width=1200
)

fig.write_html('baseline_t1t2_ACh_declining_patients.html')
fig.show()