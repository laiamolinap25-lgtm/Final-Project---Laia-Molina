...
for row, mean, df_rec_plot, common, label, color in datasets:
    for col, receptor in enumerate(receptor_cols, start=1):
        ...
        fig.add_trace(go.Scatter(
            ...
            text=region_names,
            hovertemplate='<b>%{text}</b><br>T1T2 baseline: %{x:.4f}<br>Receptor: %{y:.4f}<extra></extra>',
        ...
        fig.update_xaxes(title_text='T1T2 baseline', row=row, col=col)
...
fig.update_layout(
    title='Spearman ρ:   T1T2 baseline  vs  Receptor Density',
    height=800, width=900,
    template='plotly_white'
)

fig.write_html('baseline_t1t2_GABA_declining_patients.html')
fig.show()