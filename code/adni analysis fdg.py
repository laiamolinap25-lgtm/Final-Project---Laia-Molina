...
fdg = pd.concat(dfs, ignore_index=True)

fdg["tp_months"] = fdg["timepoint"].map(timepoint_to_months)

# Check unknown timepoints
unknown = fdg.loc[fdg["tp_months"].isna(), "timepoint"].unique()


patient_summary = (
    fdg.groupby(["group", "patient_id"])
    .agg(
...
patient_summary.to_csv(
    os.path.join(output_dir, "fdg_patient_summary_by_group.csv"),
    index=False
)
...
summary.to_csv(
    os.path.join(output_dir, "fdg_patient_summary_group_level.csv"),
    index=False
)


timepoint_summary = (
    fdg.groupby(["group", "timepoint"])
    .size()
...
timepoint_summary.to_csv(
    os.path.join(output_dir, "fdg_timepoint_summary_by_group.csv"),
    index=False
)
...
plt.title("FDG patients valid for baseline and delta analyses")
plt.legend()
plt.tight_layout()
plt.savefig(
    os.path.join(output_dir, "fdg_baseline_delta_valid_by_group.png"),
    dpi=300
)
plt.close()
...
plt.title("FDG timepoint availability by diagnostic group")
plt.tight_layout()
plt.savefig(
    os.path.join(output_dir, "fdg_timepoint_availability_by_group.png"),
    dpi=300
)
plt.close()
...
for group in visits_dist["group"].unique():
    ...
    plt.ylabel("Number of patients")
    plt.title(f"FDG visits per patient - {group}")
    plt.tight_layout()
    plt.savefig(
        os.path.join(output_dir, f"fdg_visits_per_patient_{group}.png"),
        dpi=300
    )
    plt.close()
